...
class Report():
    ...
    def initialize(self, fpath):
        ...
        # 1. Make project  directory and other sub-directories
        self.makeDirs(fpath)
        subs = list(self.objects) + ['output'] 
        for sub in subs:
            self.makeDirs(os.path.join(fpath,sub))
            
        # 2. Add packages and preamble to Document. 
    ...
    def use_arxiv(self):
        ...
        This file is needed for the arxiv-style reports. 
        '''
    ...
    def saveFigure(self, name, inPath='', savePath='', caption='', option='', override=False):
        '''
        Given an existing image file (inPath), save to (savePath) 
        ...
        if (not os.path.exists(outPng) and inPath != '') or override==True:
            print(f'Error: {name}.png file does not exist in {savePath}. \
                  Creating new png file.')
            shutil.copyfile(inPath, outPng)

        elif inPath == '' and not os.path.exists(outPng):
